<div>
    ...
    <div class="px-4 py-8 sm:px-6 lg:px-8 border sm:rounded-lg my-6 max-w-full">
        ...
        <pre class="overflow-auto bg-gray-800"><code id="code" class="text-xs text-green-400 px-2">{{ '<div id="cubaescucha-player-container" episode-title="'.$title.'" episode-owner="'.$podcast->team->name.'" podcast-url="'.route("podcast.display", ["podcast"=>$podcast->url]).'"><audio id="cubaescucha-player" type="audio/mpeg" src="'.Storage::disk("s3")->url($episode->file_name).'"></audio> <img id="cubaescucha-player-img" src="'.Storage::disk('s3')->url($podcast->thumbnail).'" alt="'.$title.'"> </div> <script src="'.asset('js/embedded-player.js').'"></script>' }}</code></pre>
        <script>

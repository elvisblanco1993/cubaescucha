<div class="mt-12 sm:flex items-center justify-between mx-4 md:mx-8 text-blueGray-800">
    ...
    <div class="w-full sm:w-1/2 text-sm font-light text-left sm:text-right py-3">
        <a href="https://help.cubaescucha.com" class="mx-2">{{ __('Help') }}</a>
        <a href="{{ config('app.url') . '/privacy-policy' }}" class="mx-2">{{ __('Privacy') }}</a>
        <a href="{{ config('app.url') . '/terms-of-service' }}" class="mx-2">{{ __('Terms') }}</a>
        <a href="https://www.paypal.com/donate?business=XV9WBWPSYN7WG&item_name=Your+donation+will+be+purposed+to+develop+and+maintain+cubaescucha.com.&currency_code=USD" target="__blank" class="ml-2 px-2 bg-yellow-400 rounded">{{ __('Donate') }}</a>
    </div>
